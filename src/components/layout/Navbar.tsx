  ...
  return (
    <header className="sticky top-0 z-50 w-full border-b border-zinc-200/60 bg-white/90 backdrop-blur-md text-zinc-900 shadow-sm transition-all">
      <div className="container mx-auto px-4 h-20 flex items-center justify-between">
        ...
        {/* Right: Login & Admin Conditional */}
        <div className="flex items-center gap-4 text-[13px] font-bold uppercase tracking-wider">
          {!session ? (
            <Link href="/login" className="hidden sm:block text-zinc-500 hover:text-black transition-colors px-6 py-2.5 rounded-full border border-zinc-200 hover:bg-zinc-50">
              Giriş
            </Link>
          ) : (
